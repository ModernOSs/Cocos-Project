...
bool SelectScene::init() {
	...
	for (int i = 0; i < Global::score; i++) {
		...
		if (i == 0) GameItem = MenuItemImage::create(imageFileName, imageFileName, [&](Ref* pSender) {auto gamescene = Wonderland::createScene(i + 1); Director::getInstance()->replaceScene(gamescene); });
		if (i == 1) GameItem = MenuItemImage::create(imageFileName, imageFileName, [&](Ref* pSender) {auto gamescene = LevelOne::createScene(i + 1); Director::getInstance()->replaceScene(gamescene); });
		if (i == 2) GameItem = MenuItemImage::create(imageFileName, imageFileName, [&](Ref* pSender) {auto gamescene = LevelTwo::createScene(i + 1); Director::getInstance()->replaceScene(gamescene); });
